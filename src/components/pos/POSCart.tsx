...
import { Product, CartItem, ProductPackage } from '@/types';
import { PlusIcon, MinusIcon, TrashIcon, ExclamationTriangleIcon, ArrowsRightLeftIcon, TableCellsIcon, ExclamationCircleIcon, BellIcon } from '@heroicons/react/24/solid';
...
const POSCart: React.FC<POSCartProps> = ({ initialProduct }) => {
  ...
  const minQtyValidationErrors = items.reduce((acc, item) => {
    ...
    const currentBase = item.baseUnitQuantity;
  ...
  const checkStockAvailability = (
    ...
    excludeItemId?: string
      ...
      if (totalRequired > availableStock + 0.0001) {
    ...
    if (existingItemIndex >= 0) {
        ...
        const newItems = items.filter(i => i.id !== itemId).map(i => {
            if (i.id === existingItem.id) {
                return { ...i, quantity: mergedQty, baseUnitQuantity: mergedBaseQty };
            }
            return i;
        });
        setItems(newItems);
  ...
  const updateWarehouse = (itemId: string, newWarehouseId: string) => {
    ...
    if (!stockCheck.allowed) {
        setValidationError({ message: stockCheck.message || `Não é possível mover para este armazém: stock insuficiente.`, productId: fullProduct.id });
        return;
    }

    setValidationError(null);
    const existingItemInNewWarehouse = items.find(i => i.id !== itemId && i.productId === item.productId && i.selectedPackage === item.selectedPackage && i.warehouseId === newWarehouseId);
    ...
    if (existingItemInNewWarehouse) {
        ...
        const updatedItems = items
            .map(i => i.id === existingItemInNewWarehouse.id ? { ...i, quantity: mergedQty, baseUnitQuantity: mergedBaseQty } : i)
  ...
  const filteredItems = items.filter(item => item.productName.toLowerCase().includes(searchTerm.toLowerCase()) || item.productId.toLowerCase().includes(searchTerm.toLowerCase()));
    ...
    <div className="flex flex-col h-full relative">
        ...
        <div className="absolute inset-0 z-50 flex items-center justify-center bg-gray-900 bg-opacity-50 backdrop-blur-sm rounded-lg">
            <div className="bg-white p-6 rounded-xl shadow-2xl max-w-md w-full mx-4 border border-gray-200 animate-fadeInDown">
                ...
                <div className="flex flex-col gap-3">
                    <button onClick={handleConfirmMerge} className="flex items-center justify-center w-full px-4 py-3 bg-blue-600 text-white rounded-lg font-semibold hover:bg-blue-700 transition-colors shadow-sm">
                        <ArrowsRightLeftIcon className="h-5 w-5 mr-2" /> Fundir (Total: {mergeConflict.existingItem.quantity + mergeConflict.quantityToAdd})
                    </button>
                    <button onClick={handleConfirmSeparate} className="flex items-center justify-center w-full px-4 py-3 bg-white border border-gray-300 text-gray-700 rounded-lg font-semibold hover:bg-gray-50 transition-colors">
                        <PlusIcon className="h-5 w-5 mr-2" /> Nova Linha Separada
                    </button>
                </div>
                 <button onClick={() => setMergeConflict(null)} className="mt-4 text-xs text-gray-400 hover:text-gray-600 w-full text-center"> Cancelar </button>
            </div>
        </div>
      )}
      ...
      <div className="mb-3 relative">
        ...
        <input
            ...
            className="block w-full pl-9 pr-3 py-2 border border-gray-300 rounded-md leading-5 bg-white placeholder-gray-500 focus:outline-none focus:placeholder-gray-400 focus:border-primary focus:ring-1 focus:ring-primary sm:text-sm shadow-sm"
            value={searchTerm}
            onChange={(e) => setSearchTerm(e.target.value)}
        />
      </div>

      {/* NEW: DEDICATED VALIDATION SUMMARY SECTION */}
      {(hasStockErrors || hasMinQtyErrors || validationError) && (
        <div className="mb-4 space-y-2">
            {validationError && (
                <div className="p-3 text-xs bg-yellow-50 border border-yellow-200 text-yellow-800 rounded-lg flex items-center animate-fadeInDown">
                    <ExclamationTriangleIcon className="h-4 w-4 mr-2 text-yellow-600 flex-shrink-0" />
                    <span className="font-medium">{validationError.message}</span>
                </div>
            )}
            {(hasStockErrors || hasMinQtyErrors) && (
                <div className="bg-red-50 border border-red-200 rounded-lg p-3 animate-fadeInDown">
                    <div className="flex items-center mb-2">
                        <BellIcon className="h-4 w-4 text-red-600 mr-2" />
                        <h4 className="text-xs font-bold text-red-800 uppercase tracking-wider">Alertas de Carrinho ({Object.keys(stockValidationErrors).length + Object.keys(minQtyValidationErrors).length})</h4>
                    </div>
                    <ul className="space-y-1">
                        {items.map(item => {
                            const sErr = stockValidationErrors[item.id];
                            const mErr = minQtyValidationErrors[item.id];
                            if (!sErr && !mErr) return null;
                            return (
                                <li key={`summary-${item.id}`} className="text-[10px] flex items-start text-red-700">
                                    <span className="font-bold mr-1 truncate max-w-[80px]">{item.productName}:</span>
                                    <span>{sErr || mErr}</span>
                                </li>
                            );
                        })}
        ...
        {items.length === 0 ? (
          ...
          <ul className="divide-y divide-gray-200">
            {filteredItems.map((item, index) => {
                ...
                const hasError = !!stockError || !!minQtyError || (validationError?.productId === item.productId);
                const isLastItem = index === filteredItems.length - 1 && !isRemoving;
                const fullProduct = mockProducts.find(p => p.id === item.productId);
                
                const itemClasses = [
                    'py-4 transition-all duration-300 relative', 
                    isLastItem ? 'animate-fadeInDown' : '', 
                    isRemoving ? 'animate-fadeOutUp' : '', 
                    hasError ? 'cart-item-error -mx-2 px-2 rounded-md shadow-sm border border-red-200' : ''
                ].filter(Boolean).join(' ');

                const pkg = fullProduct?.packages.find(p => p.name === item.selectedPackage);
                const minBase = fullProduct?.minPurchaseQuantity || 1;
                const allowDecimals = ['KG', 'L', 'M', 'M2', 'M3'].includes(fullProduct?.baseUnit?.toUpperCase() || '') && pkg?.factor === 1;
                    ...
                    <div className="flex items-center justify-between">
                      <div className="flex-grow">
                          <div className="flex items-center">
                            <p className={`font-semibold text-sm ${hasError ? 'text-red-900' : 'text-gray-800'}`}>{item.productName}</p>
                            {hasError && <ExclamationCircleIcon className="h-4 w-4 ml-1 text-red-500" />}
                          </div>
                          <div className="flex items-center mt-1 space-x-2">
                              <span className="text-xs text-gray-600">{item.unitPrice.toFixed(2)} Kz</span>
                              {fullProduct && fullProduct.packages.length > 1 ? (
                                  <select value={item.selectedPackage} onChange={(e) => updatePackage(item.id, e.target.value)} className="text-[10px] border-gray-300 rounded shadow-sm focus:border-primary focus:ring-primary py-0.5 px-1 bg-white">
                                      {fullProduct.packages.map(p => (<option key={p.name} value={p.name}>{p.name}</option>))}
                                  </select>
                              ) : (
                                  <span className="text-[10px] text-gray-500 bg-gray-100 px-1.5 py-0.5 rounded">{item.selectedPackage}</span>
                              )}
                          </div>
                          
                          {/* ENHANCED PER-ITEM ERROR DISPLAY */}
                          <div className="mt-1.5 space-y-1">
                            {stockError && (
                                <div className="text-[10px] text-red-600 font-bold bg-red-50 border border-red-100 px-2 py-0.5 rounded-md flex items-center w-fit">
                                    <ExclamationTriangleIcon className="h-3 w-3 mr-1" /> {stockError}
                                </div>
                            )}
                            {minQtyError && (
                                <div className="text-[10px] text-orange-600 font-bold bg-orange-50 border border-orange-100 px-2 py-0.5 rounded-md flex items-center w-fit">
                                    <ExclamationCircleIcon className="h-3 w-3 mr-1" /> {minQtyError}
                                </div>
                            )}
                          </div>
                      </div>
                      <div className="flex items-center space-x-2 ml-4">
                          <button onClick={() => updateQuantity(item.id, Math.max(minPkgQty, item.quantity - (allowDecimals ? 0.5 : 1)))} className="p-1 rounded-full bg-gray-200 hover:bg-gray-300 flex-shrink-0"><MinusIcon className="h-4 w-4"/></button>
                          <QuantityInput
                            min={minPkgQty}
                            step={allowDecimals ? "0.01" : "1"}
                            value={item.quantity}
                            onChange={(val) => updateQuantity(item.id, val)}
                            className={`w-14 text-center border rounded text-xs py-1 focus:ring-primary focus:border-primary ${hasError ? 'border-red-300 bg-red-50 text-red-900' : 'border-gray-300'}`}
                          />
                          <button onClick={() => updateQuantity(item.id, item.quantity + (allowDecimals ? 0.5 : 1))} className="p-1 rounded-full bg-gray-200 hover:bg-gray-300 flex-shrink-0"><PlusIcon className="h-4 w-4"/></button>
                          <button onClick={() => removeItem(item.id)} className="text-gray-400 hover:text-red-600 transition-colors ml-1"><TrashIcon className="h-5 w-5"/></button>
                      </div>
                    </div>
                    {fullProduct?.trackStock && (
                      <div className="mt-2 flex flex-wrap items-center gap-y-2 bg-gray-50 p-1.5 rounded-md border border-gray-100">
                        <label htmlFor={`warehouse-${item.id}`} className="text-[10px] text-gray-500 mr-2 font-medium">Local:</label>
                        <select
                          id={`warehouse-${item.id}`}
                          value={item.warehouseId}
                          onChange={(e) => updateWarehouse(item.id, e.target.value)}
                          className="text-[10px] border-gray-200 rounded shadow-sm focus:border-primary focus:ring-primary py-0.5 pr-6 bg-white"
                        >
                          {mockWarehouses.filter(w => w.id === item.warehouseId || fullProduct.stockLevels.some(s => s.warehouseId === w.id && s.quantity > 0)).map(wh => {
                              const s = fullProduct.stockLevels.find(sl => sl.warehouseId === wh.id)?.quantity || 0;
                              return <option key={wh.id} value={wh.id}>{wh.name} ({s})</option>;
                          })}
                        </select>
                      </div>
                    )}
      ...
      <div className="border-t pt-4 mt-4 bg-white sticky bottom-0">
        <div className="flex justify-between items-center text-xl font-bold">
          <span className="text-gray-600 text-sm font-medium">Subtotal Carrinho</span>
          <span className={totalValueExceeded ? 'text-red-600 animate-pulse' : 'text-gray-900'}>{totalValue.toFixed(2)} Kz</span>
        </div>
        {totalValueExceeded && <p className="text-red-600 text-[10px] mt-1 font-bold">Excedeu o limite máximo de {CART_LIMITS.MAX_TOTAL_VALUE.toFixed(2)} Kz.</p>}
        <button 
          disabled={items.length === 0 || totalValueExceeded || !!mergeConflict || hasStockErrors || hasMinQtyErrors}
          onClick={handleFinalize}
          className="w-full mt-4 bg-primary text-white font-bold py-3 px-4 rounded-lg hover:bg-blue-700 transition-colors disabled:bg-gray-300 disabled:text-gray-500 disabled:cursor-not-allowed shadow-md">
          {hasStockErrors || hasMinQtyErrors ? 'Corrija os erros para finalizar' : 'Finalizar Venda'}
        </button>
      </div>
    </div>
  );
